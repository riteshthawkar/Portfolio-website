...
const education = profile.education
  .map((item) =>
    [
      ...
      "  \\end{tabular*}",
      "  \\begin{itemize}",
      "    \\item \\textbf{GPA:} " + latex(item.gpa.replace(/\s*\/\s*/g, "/")),
      "    \\item \\textbf{Relevant Coursework:} " + latex(item.keyCourses.join(", ")),
      "  \\end{itemize}",
    ].join("\n"),
  )
  .join("\n")
...
const document = [
  ...
  "\\fancyfoot[C]{\\small Ritesh Thawkar \\enspace | \\enspace Page \\thepage}",
  ...
  "\\end{itemize}",
  "",
  "\\section*{Research Service \\& Languages}",
  "\\textbf{Research Service:} " + latex(profile.cv.researchService.join(", ")) + "\\\\",
